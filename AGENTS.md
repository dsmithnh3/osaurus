...
- Repo navigation: [`docs/DEVELOPER_MAP.md`](docs/DEVELOPER_MAP.md) → [`docs/agent-stubs/<topic>.md`](docs/agent-stubs/) → feature docs under `docs/` → `Packages/OsaurusCore/` code; table of contents: [`docs/INDEX.md`](docs/INDEX.md). Build, test, lint, and architecture rules belong in [`CLAUDE.md`](CLAUDE.md) only; if that link breaks CI, see [`docs/DOC_LINKS_AND_CI.md`](docs/DOC_LINKS_AND_CI.md) until `CLAUDE.md` is committed.
- To compare a feature branch to `main` as “what changed on this branch since it diverged,” use a three-dot diff: `git diff main...HEAD` or `git diff origin/main...HEAD`; for a single file, `git diff main...HEAD -- path/to/file`.
